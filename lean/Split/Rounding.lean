/-
  Rounding lemmas: CEIL on money IN, FLOOR on money OUT ⇒ the vault can only ever
  STRAND bounded dust, never DRAIN. These feed T_RoundingMonotone and T_ResidualBound.
-/
import Split.State
namespace Split
...
theorem ceil_ge (a b : Nat) (hb : 0 < b) : a ≤ ceilDiv a b * b := by
  ...
  unfold ceilDiv
  rw [if_neg hbne]
  -- goal: a ≤ ((a + b - 1) / b) * b. Treat the quotient as opaque via div_add_mod.
  have hdm := Nat.div_add_mod (a + b - 1) b   -- b * ((a+b-1)/b) + (a+b-1)%b = a+b-1
...
theorem floor_le (a b : Nat) : floorDiv a b * b ≤ a := by
  ...
  · rw [if_pos hb]; simp
  · rw [if_neg hb]
    have hdm := Nat.div_add_mod a b   -- b * (a/b) + a%b = a
    have hcomm : (a / b) * b = b * (a / b) := Nat.mul_comm _ _
    omega

/-- Dust from a FLOOR pro-rata split is bounded by `basis` units: the remainder of
    floor division is strictly less than the divisor. -/
...
theorem div_add_le (x y b : Nat) (hb : 0 < b) : (x + y) / b ≤ x / b + y / b + 1 := by
  -- x < b*(x/b+1), y < b*(y/b+1) ⇒ x+y < b*(x/b+y/b+2) ⇒ (x+y)/b < x/b+y/b+2.
  -- Work with the expanded forms so omega's atoms are {b*(x/b), b*(y/b), b}.
  ...
  -- (x+y)/b < x/b+y/b+2 because x+y < b*(that), via Nat.div_lt_iff_lt_mul
  have hlt : (x + y) / b < x / b + y / b + 2 := by
    rw [Nat.div_lt_iff_lt_mul hb]; rw [Nat.mul_comm] at hsum; exact hsum
  omega

/-- Sub-additivity of floor division up to 1: `⌊(x+y)/b⌋ ≤ ⌊x/b⌋ + ⌊y/b⌋ + 1` (b>0). -/
...
/-- Division is monotone in the numerator (core has no named lemma here). -/
...
/-- Floor of a floor-pro-rata split is monotone in the numerator factor. -/
