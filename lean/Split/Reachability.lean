/-
  Reachability: states produced by finite valid transition sequences from `init`.
  WF is proved to be an inductive invariant (holds at init, preserved by every step).
  This is the backbone of all theorems — ZERO `sorry`.
-/
import Split.State
...
/-- WF holds at genesis. All supplies/claims are 0, not yet settled. -/
theorem wf_init (strike maturity exerciseEnd : Nat) : WF (init strike maturity exerciseEnd) := by
  constructor <;> intro h <;> simp [init] at h ⊢

/-- mint preserves WF. Pre-settle stays pre-settle; collat,pSupply,nSupply all += q. -/
...
/-- exercise preserves WF. collat,nSupply drop by q; strikeBal rises; pSupply fixed. -/
...
theorem wf_step_settle {s s' : State} (h : WF s) (hstep : settle s = some s') : WF s' := by
  ...
  · rename_i hg
    ...
    have hcu := h.preNoCredU hns
    split at hstep
    · -- pSupply = 0 : residual routed to the recipient (credited)
      rename_i hp0
      injection hstep with hs'; subst hs'
      constructor <;> intro hc <;> simp_all [floorDiv]
    · -- pSupply > 0 : frozen pool = current pool, no credits yet
      rename_i hp0
      injection hstep with hs'; subst hs'
...
/-- redeemP preserves WF — the hard case. Uses floor super-additivity. -/
theorem wf_step_redeemP {s s' : State} {q : Nat} (h : WF s)
  ...
  · rename_i hg
    ...
    injection hstep with hs'; subst hs'
    -- super-additivity: floor(q*c0/P0) + floor((p-q)*c0/P0) ≤ floor(p*c0/P0).
    -- These chain old backW/resW/liveW (stated on floor(p*c0/P0)) to the new ones
    -- (stated on floor(q*c0/P0)=cw and floor((p-q)*c0/P0)).
    ...
    have hsuperU :
      ...
      rwa [hsplitU] at this
    -- sub-additivity (up to 1): floor(p*c0/P0) ≤ cw + floor((p-q)*c0/P0) + 1.
    -- This drives liveW (the residual-liveness / dust direction). The +1 dust unit is
    -- absorbed because q ≥ 1 grows the dust slack (P0-(p-q)) = (P0-p)+q by ≥ 1.
    have hsubW :
    ...
    -- dust slack identity: after burning q P, the per-unit dust budget grows by q.
    -- (P0 - (p - q)) = (P0 - p) + q, since q ≤ p ≤ P0.
    have hdust : s.pSupplyAt - (s.pSupply - q) = (s.pSupplyAt - s.pSupply) + q := by omega
    -- the new state is settled, so every pre-settle field is vacuously discharged
    -- (its hypothesis `¬settled` contradicts hns); the post-settle inequalities are
    -- closed by omega via super/sub-additivity facts + the dust slack identity.
    -- drop the nonlinear product equalities: they are already consumed into the
    -- super/sub-additivity facts and only add opaque atoms that distract omega.
    clear hsplitW hsplitU
    -- The new state is settled (settled field unchanged). `simp only [hns, ...]` both
    -- (a) reduces the structure-projection atoms `{…}.pSupply`/`.pSupplyAt` to
    --     `s.pSupply - q` / `s.pSupplyAt` so omega can connect the dust atoms, and
    -- (b) collapses each pre-settle field's `¬settled` hypothesis to `False`
    --     (via `not_true_eq_false`), closing those goals outright.
    -- The post-settle inequalities are then closed by omega via the super/sub-additivity
    -- facts (hsuperW/U, hsubW/U) + the dust slack identity (hdust).
    refine ⟨?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩ <;> intro hc <;>
      simp only [hns, not_true_eq_false] at hc ⊢ <;>
      omega
  · exact absurd hstep (by simp)

/-- claimW preserves WF. Holdings and credits drop in lock-step (zero-before-send). -/
...
/-- tick (advance `now`) preserves WF: only `now` changes; no WF field reads it. -/
